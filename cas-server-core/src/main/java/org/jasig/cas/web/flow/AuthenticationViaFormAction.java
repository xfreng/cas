...
public class AuthenticationViaFormAction {
    ...
    // 验证码参数：
    private static final String code = "sRand";
    ...
    public final Event submit(final RequestContext context, final Credential credential,
        ...
        String sRand = (String) WebUtils.getHttpServletRequest(context).getSession().getAttribute(code);

        if (credential instanceof UsernamePasswordCredential) {
            UsernamePasswordCredential usernamePasswordCredential = (UsernamePasswordCredential) credential;
            if (StringUtils.isEmpty(usernamePasswordCredential.getUsername())) {
                logger.warn("username is empty {}", usernamePasswordCredential.getUsername());
                messageContext.addMessage(new MessageBuilder().error().code("required.username").build());
                return newEvent(ERROR);
            }
            if (StringUtils.isEmpty(usernamePasswordCredential.getPassword())) {
                logger.warn("password is empty {}", usernamePasswordCredential.getPassword());
                messageContext.addMessage(new MessageBuilder().error().code("required.password").build());
                return newEvent(ERROR);
            }
            if (StringUtils.isEmpty(usernamePasswordCredential.getRand())) {
                logger.warn("rand is empty {}", usernamePasswordCredential.getRand());
                messageContext.addMessage(new MessageBuilder().error().code("required.rand").build());
                return newEvent(ERROR);
            }
            if (!sRand.equalsIgnoreCase(usernamePasswordCredential.getRand())) {
                logger.warn("rand input error {}", usernamePasswordCredential.getRand());
                messageContext.addMessage(new MessageBuilder().error().code("required.rand.error").build());
                return newEvent(ERROR);
            }
        }
