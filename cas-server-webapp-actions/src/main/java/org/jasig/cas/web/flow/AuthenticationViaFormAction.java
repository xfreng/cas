...
public class AuthenticationViaFormAction {
     ...
     * The rand param name.
     */
    private static final String RAND = "sRand";
    ...
    public final Event submit(final RequestContext context, final Credential credential,
        ...
        return validatorRand(context, credential, messageContext);
    }

    private Event validatorRand(final RequestContext context, final Credential credentials, final MessageContext messageContext) {
        ...
        UsernamePasswordRandCredential upc = (UsernamePasswordRandCredential) credentials;
        //用户 输入的验证码
        String submitAuthRand = upc.getRand();
        if (StringUtils.isEmpty(submitAuthRand) || StringUtils.isEmpty(sRand)) {
            messageContext.addMessage(new MessageBuilder().error().code("rand.required").build());
            return newEvent(AbstractCasWebflowConfigurer.TRANSITION_ID_ERROR);
        }
        if (submitAuthRand.equalsIgnoreCase(sRand)) {
            if (isRequestAskingForServiceTicket(context)) {
                return grantServiceTicket(context, credentials);
            }
            return createTicketGrantingTicket(context, credentials, messageContext);
        } else {
